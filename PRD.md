...
- **Security:** connected instances are accessed with an n8n API key scoped to read-only wherever that scoping is technically available — but read-only API-key scopes (`execution:read`, `workflow:read` without write/delete) are an **n8n Enterprise-only feature**. On Community Edition (self-hosted, free) and standard Cloud plans — the tier the primary persona (§3) is actually on — an API key is full-access by default, so n8n itself cannot enforce read-only for most real users of this tool. The guarantee is therefore enforced at the application layer instead: Insight's own code calls only a documented allowlist of read endpoints (`GET /executions`, `GET /executions/{id}`) and never calls a write or delete endpoint, regardless of what the key it's handed could technically do — the system never requests or stores write credentials for a monitored instance. Secrets are redacted before any data reaches the LLM (FR-3) or persistent storage. The public paste-and-diagnose page (FR-12) also accepts an API key from an anonymous user for their own instance — this key is held in memory for the single request only, never logged, never written to Postgres, and transmitted over HTTPS only; the page states this plainly next to the input field. The public page is also rate-limited per IP (a low requests-per-minute cap is sufficient at this project's scale) — since Groq's free tier is capped at the organization level, not per-key (§6.2), an unauthenticated endpoint with no rate limit is a standing cost/availability risk shared across every other caller of the same LLM quota, not just a hardening nicety.
...
| Neon **read-only** role connection string | Vercel env var, server-side only | Dashboard queries |
...
| Connected instance (FR-1a/b) | Persisted in Neon as ciphertext (AES-256-GCM, key from env, per-row nonce) on the `connected_instances` table — not as an n8n credential, since instances are added through the dashboard at runtime rather than hand-created one at a time. The write itself goes through n8n (§6.6's "Connecting an instance" sequence), not directly from Next.js — see the note below on why the dashboard's own Postgres role stays read-only. Decrypted in-workflow only for the duration of a read call. |

Both categories are subject to the app-layer read-endpoint allowlist in §5: Insight's code path never constructs a request to a write or delete endpoint, regardless of what the key it holds could technically do.
...
3. That workflow validates the key (a single low-privilege read call, e.g. fetching the instance's own user info), generates the ingest token, encrypts the API key, and writes or updates the `connected_instances` row.
4. Revoking an instance follows the same path: dashboard → manage-instance workflow → row update (key erased, token invalidated).

This is the one write path into `connected_instances` and it stays inside n8n on purpose: it means n8n remains the **sole writer** to Postgres (consistent with §6.2a), and the dashboard's own Neon role (§6.5) can stay strictly read-only for every display query without exception.
...
| 2 | Does the dashboard read Postgres directly, or through n8n? | Directly, from Next.js server-side using a Neon **read-only** role, for every display query. The one exception is registering or revoking an instance, which writes through the dedicated "manage-instance" n8n workflow (§6.6) rather than from Next.js — so n8n stays the sole writer to Postgres (§6.2a) and the read-only role label in §6.5 stays literally true. | Proposed |
...
| Handling a third party's execution data, which may contain real business data | App-layer read-only enforcement via a documented endpoint allowlist, since n8n's own read-only API-key scoping is Enterprise-only and unavailable to most real users of this tool (§5); secret redaction before any LLM call (FR-3); minimal retention; stated plainly to connecting users |
